...
class PacienteProviderCompletoInterface(ABC):
    """
    ...
    async def listar_pacientes(self) -> List[Dict[str, Any]]:
        ...
        ser incluídos. Cada item da lista contém:
            - prontuario: número do prontuário (int, chave primária)
            - telefone:   telefone de contato (str)
            - cidade:     cidade de residência (str)
            - estado:     estado de residência (str)
        """
        pass
    ...
    async def verificar_prontuario_existe(self, numero_prontuario: int) -> bool:
        """
        Verifica se um prontuário existe e está ativo no AGHU.
    ...
    async def verificar_solicitacao_existe(
        ...
        Retorna True apenas se ambas as condições forem satisfeitas e o
        registro não estiver deletado logicamente.
    ...
    async def buscar_exames_solicitacao(
        ...
        Cada item da lista contém:
            -- de Exame --
            - codigo_exame:         código do exame (str)
            - nome_exame:           nome do exame (str)
            -- de Solicitacao --
            - data_retorno:         data de retorno da solicitação (date)
            - unidade_solicitante:  unidade que originou a solicitação (str)
            -- de ExameSolicitado --
            - data_solicitacao:     data em que o exame foi solicitado (date)
            - status_atribuicao:    status atual de atribuição (str ou None)
            - funcionario_atribuido: id do funcionário atribuído (int ou None)
            -- calculado --
            - tem_vagas:            se há vagas disponíveis (bool)
